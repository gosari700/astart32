...
// 항상 홀수 문장(짝수 인덱스)으로 시작하도록 보장
if (sentenceIndex % 2 === 1) {
    // 현재 인덱스가 홀수(짝수 문장)이면, 다음 짝수 인덱스(홀수 문장)로 이동
...
const SENTENCE_VERTICAL_ADJUSTMENT = -41; // 기존 -36에서 5px 더 감소시켜 첫 번째 문장을 추가로 위로 올림
const ANSWER_OFFSET_Y = 57; // 62에서 57로 변경하여 두 번째 문장을 5px 위로 올림
...
function startFireworks(sentenceTextForFireworks, globalSentenceIndex, explosionX, explosionY) {
    ...
    let answerTextForLayout = "";
    const isNewSentenceQuestion = globalSentenceIndex % 2 === 0;
    roleOfNewSentence = isNewSentenceQuestion ? 'question' : 'answer';
    
    // 질문 및 답변 문장 둘 다 표시하기 위한 준비 작업
    if (roleOfNewSentence === 'question') {
        // 홀수 문장(Question) 폭발 시 이후 짝수 문장(Answer)도 표시
        // 현재 홀수 문장에 해당하는 짝수 문장 찾기 (다음 문장)
        if (globalSentenceIndex + 1 < sentences.length) {
            answerTextForLayout = sentences[globalSentenceIndex + 1];
            console.log(`홀수 문장(${globalSentenceIndex+1}) 폭발 - 해당 짝수 문장(${globalSentenceIndex+2})도 표시`);
        } else {
            answerTextForLayout = "";
        }
        ...
        // 짝수 문장(Answer) 폭발 시 이전 홀수 문장(Question)도 표시 (기존 로직)
        ...
        console.log(`짝수 문장(${globalSentenceIndex+1}) 폭발 - 해당 홀수 문장(${globalSentenceIndex})도 표시`);
    ...
    const baseRadius = 51.2 * 0.88; const maxRadius = 120.96 * 0.88 * 0.95 * 0.93 * 0.95 * 0.94 * 0.97; // Adjusted maxRadius - 7% 축소 + 5% 축소 + 추가 6% 축소 + 3% 추가 축소
  ...
  if (fireworksState.phase === "explode") {
    ...
    if (progress >= 1) {
        ...
        let playAudioForThisSentence = false;        if (roleOfNewSentence === 'question') {
            ...
            // 홀수 문장 폭발 시에도 짝수 문장 표시 (다음 문장)
            const answerIndexOfThisQuestion = newSentenceIndex + 1;
            if (answerIndexOfThisQuestion < sentences.length) {
                console.log(`홀수 문장(${newSentenceIndex+1}) 폭발 시 짝수 문장(${answerIndexOfThisQuestion+1}) 설정`);
                const [aL1, aL2] = splitSentence(sentences[answerIndexOfThisQuestion], false);
                currentAnswerSentence = {line1: aL1, line2: aL2};
                currentAnswerSentenceIndex = answerIndexOfThisQuestion;
                showPlayButton = true;
            } else {
                // 다음 문장이 없는 경우
    ...
    enemies.forEach((e, ei) => {
      ...
      const coreBulletHeight = b.h * (1 - 2 * collisionPaddingFactor);      if (coreBulletX < e.x + e.w && coreBulletX + coreBulletWidth > e.x &&
        ...
        if (!sentenceActive) {
            ...
            // 항상 홀수 문장만 폭발하도록 조정 (인덱스는 0부터 시작하므로 짝수 인덱스가 홀수 문장)
            // 현재 인덱스를 기반으로 다음 홀수 문장(짝수 인덱스)으로 이동
            sentenceIndex = (sentenceIndex + 1) % sentences.length;
            
            // 다음 문장이 짝수 문장(홀수 인덱스)이면 한 번 더 증가하여 홀수 문장으로
            if (sentenceIndex % 2 === 1) {
                sentenceIndex = (sentenceIndex + 1) % sentences.length;
            }
